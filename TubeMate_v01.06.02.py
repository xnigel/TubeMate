...
# 这是一个硬编码的简单密码，请务必修改为保护性较强的密码
PASSWORD = "nigel"

class TubeMate:
    def __init__(self, root):
        self.root = root
        ver = "01.06.02" # Updated version number
        yr = "2025.09.26"
        ...
        self.last_clipboard = ""
        self.stop_auto_check = threading.Event()

        self.setup_ui()
        self.load_settings_into_ui()
        
        # Start the clipboard monitoring thread
        self.auto_check_thread = threading.Thread(target=self.check_clipboard_periodically)
    ...
    def setup_ui(self):
        ...
        self.path_entry = tk.Entry(settings_frame, width=30)
        self.path_entry.grid(row=0, column=1, columnspan=3, padx=5, pady=5, sticky="WE")

        self.button_select = tk.Button(settings_frame, text="Select", command=self.select_path)
        self.button_select.grid(row=0, column=4, padx=5, pady=5, sticky="W")
        ...
        self.quality_menu = ttk.Combobox(settings_frame, textvariable=self.quality_var, state="readonly", width=7)
        self.quality_menu.grid(row=1, column=4, padx=5, pady=5, sticky="W")

        # New: Auto/Manual switch
        tk.Label(settings_frame, text="Mode:", bg="#f0f0f0").grid(row=2, column=0, padx=(0, 5), pady=5, sticky="E")
        self.auto_var = tk.BooleanVar(value=self.config["auto_mode"])
        self.auto_switch = ttk.Checkbutton(settings_frame, text="Auto Mode", variable=self.auto_var, command=self.toggle_auto_mode)
        self.auto_switch.grid(row=2, column=1, columnspan=2, padx=5, pady=5, sticky="W")
        ...
        button_clear_url = tk.Button(url_frame, text="Clear URLs", width=8, command=self.clear_url_fields)
        button_clear_url.grid(row=0, column=1, padx=2, pady=2, sticky="E")

        self.url_entries = []
        for i in range(MAX_WORKERS):
            entry = tk.Entry(url_frame, width=50)
            entry.grid(row=i+1, column=0, padx=5, pady=2, sticky="EW")
            self.url_entries.append(entry)

            # Add paste button for each row
            paste_button = tk.Button(url_frame, text="Paste", width=8, command=lambda e=entry: self.paste_from_clipboard(e))
            paste_button.grid(row=i+1, column=1, padx=2, pady=2, sticky="EW")

        url_frame.grid_columnconfigure(0, weight=1)

        tk.Button(url_frame, text="Go!", width=16, command=self.add_tasks, bg="#4CAF50", fg="white").grid(row=MAX_WORKERS + 1, column=0, columnspan=2, padx=2, pady=2, sticky="E")
        ...
        button_clear_log = tk.Button(tasks_frame, text="clear logs", width=8, command=self.clear_logs)
        button_clear_log.pack(side=tk.BOTTOM, anchor="e", padx=5, pady=5)
    ...
    def toggle_auto_mode(self):
        self.config["auto_mode"] = self.auto_var.get()
        if self.config["auto_mode"]:
            root.clipboard_clear()
            # Disable manual paste buttons and URL entry fields
            for entry in self.url_entries:
                # entry.config(state="readonly")
                entry.config(state="normal")
        else:
            # Enable manual paste buttons and URL entry fields
            for entry in self.url_entries:
                entry.config(state="normal")
                
    def check_clipboard_periodically(self):
        """Continuously checks the clipboard for new URLs."""
        while not self.stop_auto_check.is_set():
            if self.config["auto_mode"]:
                try:
                    current_clipboard = self.root.clipboard_get()
                    if current_clipboard != self.last_clipboard:
                        self.last_clipboard = current_clipboard
                        video_id = self.extract_video_id(current_clipboard)
                        if video_id:
                            self.root.after(0, lambda: self.handle_auto_download(current_clipboard))
                except tk.TclError:
                    pass
            time.sleep(1)

    def handle_auto_download(self, url):
        """Finds the first empty entry and adds a task for the new URL."""
        for entry in self.url_entries:
            if not entry.get():
                entry.delete(0, tk.END)
                entry.insert(0, url)
                self.add_tasks_from_entry(entry)
                break
    
    def add_tasks_from_entry(self, entry_widget):
        """Adds a single task from a specific entry widget, used by auto mode."""
    ...
    def load_settings_into_ui(self):
        ...
        # Load the auto mode setting
        self.auto_var.set(self.config["auto_mode"])
    ...
    def extract_video_id(self, url):
        """
        Modified to correctly handle both standard and shortened YouTube URLs,
        and to strip any query parameters like ?si=...
        """
        ...
        for idx, url in enumerate(urls):
            if url:
                ...
                task_info = {
                    ...
                    "quality": self.quality_var.get(),
                    "path": self.path_entry.get(),
                    "entry_widget": self.url_entries[idx]
                }
                self.download_queue.put(task_info)
                self.create_task_ui(task_info)
                self.start_next_download()
    ...
    def download_video(self, task):
        ...
        try:
            ...
            print(f"Error during download for URL {task['url']}: {e}") # Print error to console
    ...
    def import_config(self):
        try:
            ...
            messagebox.showwarning("Configuration Import", "Imported configuration file is missing the 'auto_mode' setting. Defaulting to manual mode.")
            self.config["auto_mode"] = DEFAULT_CONFIG["auto_mode"]
            self.load_settings_into_ui()
...
if __name__ == "__main__":
    root = tk.Tk()
    root.withdraw()
    root.clipboard_clear()  # empty clipboard
    set_window_icon()

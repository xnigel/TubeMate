...
import base64
import yt_dlp
import time # Added for the new feature
...
DEFAULT_CONFIG = {
    ...
    "auto_mode": False  # New configuration for auto mode
}
...
class TubeMate:
    def __init__(self, root):
        self.root = root
        ver = "01.04.00" # Updated version number
        yr = "2025.09.09" # Updated date
        ...
        self.last_clipboard = "" # Variable to track clipboard content
        self.stop_auto_check = threading.Event() # Event to stop the monitoring thread
        ...
        self.auto_check_thread.daemon = True # Daemon thread exits when the main program exits
    ...
    def setup_ui(self):
        ...
        tk.Button(url_frame, text="Go!", width=10, command=self.add_tasks, bg="#4CAF50", fg="white").grid(row=MAX_WORKERS + 1, column=0, pady=5, sticky="E")
    ...
    def toggle_auto_mode(self):
        ...
        if self.config["auto_mode"]:
            ...
            # The 'Go!' button is still useful for clearing entries in manual mode
        else:
    ...
    def export_config(self):
        ...
        self.config["auto_mode"] = self.auto_var.get() # Save the auto mode state
        if self.config["format"] == "mp4":
    ...
    def import_config(self):
        try:
            ...
            # Handle cases where the imported config file is missing the new key
    ...
    def load_config(self):
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                    config = json.load(f)
                    # Add new keys with default values if they are missing
